@echo off
REM ============================================================
REM  Thunderstore Archive - Native launch (double-click)
REM  Creates/activates a venv, installs dependencies, starts the
REM  server, and opens the browser. Docker remains available as
REM  an option via compose.bat / update.bat.
REM ============================================================
cd /d "%~dp0"
...
if not exist "%VENV_DIR%\Scripts\python.exe" (
    ...
    if errorlevel 1 (
        echo FAILED to create the venv - is Python 3.11/3.12 installed?
        pause
        exit /b 1
    )
)
...
echo.
echo === Thunderstore Archive - starting ===
